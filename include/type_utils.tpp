...
namespace SirVariant {
	...
	struct function_arg {
		using type =
		    typename std::tuple_element<n, typename function_traits<Func>::argument_types>::type;
	};
	...
	struct impl_max_size {
		constexpr static size_t size = sizeof(T) > max ? impl_max_size<sizeof(T), Types...>::size
	...
	struct impl_max_align {
		constexpr static size_t size = alignof(T) > max ? impl_max_align<alignof(T), Types...>::size
